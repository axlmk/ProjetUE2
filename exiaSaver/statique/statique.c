...
void  lireFichier(ExiaImage* img, char* nomFichierPBM)                // lire le fichier dans la structure ExiaImage
{
        ...
        if(ligne[0] != '#')         // si la ligne n'est pas un commentaire
        {
            ...
            else // on lit une ligne du contenu
            {
                ...
                for(j=0; j< img->resX; j++)
                {
                    ...
                    posSuivante = posSuivante + strcspn(&ligne[posSuivante], " ");            // recherche prochain blanc
                    posSuivante = posSuivante + strcspn(&ligne[posSuivante], "0123456789");   // recherche prochain chiffre
                }
                numeroLigneLue++;
            }
        }
...
{
    system("clear"); // A REMPLACER par system("cls"); en Windows
    int i, j;
    for(i=0; i< img->resY; i++)
    {
        for(j=0; j< img->resX; j++)
